...
sumatriptan against the subcutaneous-only Imitrex injection SPL.

The three earlier routes each widened what `F` could mean — absent (`transdermal`), none (`im`),
full (`oral`), partial (`rectal`) — and the pattern was beginning to look like a law. **`sc` broke
it by adding nothing.** Subcutaneous tissue drains systemically exactly as muscle does, so an SC `F`
is absorption completeness with no first-pass term: the _same_ category as `im`, to the letter. It
is the first clinical route to share both an engine input type and an `F` category with a route
already shipped.

The design consequence is the deliverable: **`im` and `sc` now share one schema**
(`InjectedDepotRouteSchema`, renamed from `ImRouteSchema`) where `rectal` keeps its own, making the
rule legible — _a separate schema means a different quantity; a shared schema means the same
quantity_. The rejected alternative was a copy-pasted `ScRouteSchema`, which would have needed a doc
comment implying a distinction the physiology does not contain — the manufactured-distinction
failure the unmerged variability bands and the un-decomposed rectal `F` each refuse in their own
layer. What actually differs between IM and SC is `ka` (fat is less perfused than muscle), and a
difference in a field's _value_ is not a difference in what the field _means_. Recorded so IM's
lesson ("a shared input type doesn't carry every clinical fact") does not harden into "a new route
always means a new fact."

Sumatriptan is also **the first 2-comp compound whose Q and Vp are derived rather than cited** —
hence the new `derived_from_half_lives` sentinel. The label reports four quantities from one
9-subject study (Vc, CL, α t½ 15 min, β t½ 115 min), exactly enough to pin the model by algebra with
nothing invented; Cosson 1999 corroborates (CL to 0.6%) and is deliberately **not averaged in**. A
1-comp reading fails its own ceiling test (`F·D/V` = 29 ng/mL against a reported Cmax of 74), which
is what forced 2-comp — the ceiling test diagnosing the _model_ rather than rejecting the compound.

**Oral was drafted and then cut, and that is the most transferable part.** It was wanted because the
97%-vs-15% contrast is what makes SC's `F` category legible. It failed the magnitude check: no
single first-order `ka` reproduces both reported oral numbers (matching Tmax gives 5.4 ng/mL against
a reported 18; matching Cmax needs ka ≈ 1.0/h and puts Tmax at a third of its value). A ka sweep
showed Tmax falls monotonically — one root, so `kaFromTmax2c` is correct and oral absorption is
genuinely biphasic, exactly as Cosson fits it (first-order _then_ zero-order). Omission over a
caveated curve, because a 3.3×-low oral line beside an accurate SC one would show a ~56× peak ratio
where the truth is ~17×: **a residual is tolerable, but not when the wrong quantity is the teaching
point.** The contrast survives in prose, as ketamine's ~64%/~17% does.

**The route-ternary fallthrough recurred a third time and was caught only by launching the app.**
`PeakNote` printed _"An **oral** dose rises as it is absorbed…"_ under a subcutaneous curve, with 602
tests green — on a component whose own doc comment describes that exact bug. Adding a fifth ternary
arm would have fixed this route and left the trap armed, so it became an exhaustive
`Record<DataRoute, string>`; `ModelAssumptionsNote` had the identical shape and got the identical
fix. An advisor pass also caught a **fabricated magnitude number already written into the compound's
notes** (76.5 ng/mL at 11.6 min, guessed before the check was run; measured 72.7 at 12.0) — the
theophylline trap, inside the task documenting the theophylline trap, and invisible because the route
_passed_.
...
IM had recorded the lesson that _sharing an engine input type is not sharing a clinical route_ —
...
first read was that diazepam would show _no_ first-pass contrast — rectal ≈ oral ≈ 90%, so the hook
...
- **`ModelAssumptionsNote` printed "Oral input is a single exponential" under every curve.** Under a
  ...
  prose keyed on _nothing_ is prose keyed on whichever route was in view when it was written — and,
...
_"Study Summary"_ — a summarization, not a quotation — which is precisely the theophylline trap
...
failure, and the inverse of the clonidine rule: _the faster the input, the more compartments you
need._ Rejected alternative: taking the low end of morphine's Vd range (2.1 L/kg) lifts the ceiling
...
wrong. The narrow lesson: _a value nothing ever tested is a value nobody ever had to open the source
for_ — every route ketamine offered before IM peaked somewhere unobservable (IV bolus at C(0), an
...
- **This is the third instance of one defect class**, which is why it got a named screen rather
  ...
  screen generalises it: _does any on-screen sentence name a parameter as the CAUSE of a visible
  feature of the curve?_ That sentence is conditional on the parameter still being rate-limiting,
...
- **The overclaim the advisor caught, and the weaker statement that replaced it.** The first draft
  ...
  a rendering bug — and the _claim_ was weakened to what holds: **at the default, no edge combines
...
- **F is NOT a clone of Vd, and cloning its copy would have been the transdermal `PeakNote` defect
  ...
  template does not apply. A test asserts the F note does _not_ say "bioavailability is held".
- **The claim that shipped wrong and was walked back within the hour — the advisor retracting their
  own advice.** The first F copy read: _"Treat the two bands as one uncertainty seen twice, not two
  to add together."_ It fuses a true statement to an unsupported one. True: you cannot **attribute**
...
- **Two provenance screens run before shipping, both cheap and both capable of killing the axis.**
  A Vd stored as `derived_from_clearance` (i.e. CL/F) already embeds an F, and a stored _apparent_
...
- **Why it earns a slot next to phenytoin (a gentler nonlinearity, at a different SCALE).** Phenytoin's
  Km (7.9 mg/L) sits _below_ its therapeutic range — already past half-saturation, cliff violent (+33%
  dose more than doubles Css). Theophylline's Km (24.1) sits just _above_ its 10-15 mg/L range: it is
...
- **The isomer pair is the real payload.** Theobromine (linear, shipped) and theophylline (M-M) are both
  ...
  _model_, not the numbers. Nothing else in the set puts that boundary so close together.
- **THE ADVISOR CATCH — shipping a compound FALSIFIED prose in files the diff never had to touch.**
  ...
  was _excluded as a standalone_. The moment it shipped those became false statements inside the
...
- **What actually raised confidence was the FOUR-clearance ordering.** The FDA label's mean adult CL
  (0.65 mL/kg/min, range 0.27-1.03) is an _apparent_ CL at therapeutic levels, so under saturation it
  ...
  and their _ordering_ is what a saturable model predicts. Coherent picture, one named 15% residual.
- **THE SECOND ADVISOR CATCH — a search-summary number reached shipped prose.** A WebSearch blurb
  ...
  that second fit existing. Exactly the trap `CLAUDE.md` names (_every automated check is blind to
  whether a citation is real_), and all three gates were green over it. The discriminator the advisor
  ...
  It opened: **Dahlqvist R, Billing B, Miners JO, Birkett DJ, Ther Drug Monit 1984;6(3):290-7 (PMID 6506136)**, 11 healthy volunteers. And opening it made the argument _stronger_ than the version built
...
- **Vd is the one borrowed parameter, and that is argued, not smuggled.** `DATA_GUIDE.md` requires
  Vmax/Km/Vd from one source — but that rule exists for the _ethanol_ case, where the payload is the
  zero-order slope `Vmax/Vd`. Theophylline is the _phenytoin_ case: `Css = R0·Km/(Vmax−R0)` contains no
...
- **IV-only is the right landing, not a compromise** — IV aminophylline is a loading dose plus a
  ...
  absorbed, it simply needs a citable _adult_ ka (the one candidate found was paediatric, in a source not
...
- **Verification beyond the 512 green tests** (the standing trap): the built RK4 infusion plateau matches
  ...
  A scratch misuse of `firstOrderLimitRateMM` (it returns `ke`, 1/h — _not_ clearance) failed the check
...
- **It closed a real inconsistency; it is not decoration.** The engine takes ONE fm, so a bimodal
  compound had to anchor one phenotype and prose the other — and the half-life _range_ had to be
...
- **The identity default (advisor catch, and the best idea in the pass).** The first sketch had a
  ...
  base values ARE the default phenotype (schema-enforced), so `applyPhenotype` returns the _same
  object_ and the default render is the pre-feature compound **by construction**. The regression
...
- **Advisor corrected, on evidence.** The advisor characterised salicylate as needing an engine
  ...
  2-comp cluster (ciprofloxacin, escitalopram, sildenafil) is parked on _sourcing_, not on the
...
fm·F·D/(k_m·Vd_m)` is _independent_ of parent disposition, so the parent's slowing cancels out
...
it _fails_ (two doses run >10% above the sum of two single-dose curves), because that failure is the
...
- **The meaning of `linear` had to change, and that was the subtle part** (advisor catch). The 2c/3c
  passes were clean model-splits because those compounds are `linear: true`; an MM compound _is_
  `linear: false`, which every resolver rejected and every doc equated with "excluded from v1". So
  `linear` narrowed to its literal meaning — _may these doses be superposed?_ — the reject moved from
...
- **Oracles, given no closed form.** Pinned by what _is_ analytic: the IV-bolus implicit solution
  ...
  slower input means less saturation means _less_ exposure); and the `Km≫C` collapse onto the
...
- **A failing test that was the test's fault, not the engine's.** Multi-dose mass balance missed by
  ...
  0.28 mg, observed 0.279. Fixed in the _quadrature_ (integrate piecewise between doses); the error
...
- **The defect only _running the app_ could find.** Everything was green — 486 tests, lint, build,
  ...
  — this pass adds that they never prove _pedagogy_ either.
...
_structure + derivation_, NOT numeric correctness — magnitudes were cross-checked
...
axis 1 = was the _source value_ measured or curator-computed (the per-parameter
...
linear/superposition, first-order absorption, 70 kg _illustrative_ subject —
